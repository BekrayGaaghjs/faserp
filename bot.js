...
client.on('ready', async () => {
 ...
 client.user.setActivity(`FaseRP`, { type:'WATCHING' })
...
client.on("message", (message) =>{
    if(message.content.toLowerCase() === 'neden') {
        message.channel.send('Çünkü İkra');
    }
})

client.on("message", (message) =>{
    if(message.content.toLowerCase() === 'gizlineden') {
        message.channel.send('Ama Bu Son Yaptığınız');
    }
})
...
client.on("message", (message) =>{
  if(message.connect === 'Neden') {
    message.channel.send('Çünkü MBO')
  }
})
